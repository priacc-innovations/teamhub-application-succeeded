...
export default function ClockInOut() {
  ...
  const fetchTodayAttendance = async () => {
    ...
    try {
      const res = await axios.get(`http://44.220.159.194:8080/api/attendance/today/${user.id}`);
  ...
  const handleClockIn = async () => {
    if (!user?.id) return;
    try {
      const res = await axios.post(`http://44.220.159.194:8080/api/attendance/login/${user.id}`);
  ...
  const handleClockOut = async () => {
    if (!user?.id) return;
    try {
      const res = await axios.put(`http://44.220.159.194:8080/api/attendance/logout/${user.id}`);
