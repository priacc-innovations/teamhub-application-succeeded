...
export default function DashboardOverview() {
  ...
  const loadDashboardData = async (id: number) => {
    try {
      ...
      setMonthStats({
        present: normalized.filter(a => a.status === "present" || a.status === "late").length,
        absent: normalized.filter(a => a.status === "absent").length,
        halfDay: normalized.filter(a => a.status === "half_day").length,
        leave: normalized.filter(a => a.status === "leave").length,
      });

      // ------------------ WALLET ------------------
      const walletRes = await api.get(`/wallet/${id}`);
      setWalletBalance(walletRes.data?.current_month_earned || 0);

      // ------------------ PERFORMANCE ------------------
      const perfRes = await api.get(`/performance/${id}`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard title="Month Earnings" value={`$${walletBalance.toLocaleString()}`} icon={Wallet} color="cyan" />
      </div>
