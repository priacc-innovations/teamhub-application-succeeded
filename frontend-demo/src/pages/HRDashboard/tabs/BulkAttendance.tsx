...
export default function BulkAttendance() {
    ...
    try {
      ...
      if (isCSV) {
        ...
        parsedRecords = dataLines.map((line) => {
          const [empid, name, date, domain] = line.split(',').map((s) => s.trim());
          return { employeeId: empid, name, date, domain };
        });
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("http://44.220.159.194:8080/api/csv/save-bulk", {
        method: "POST",
  ...
  const generateCSVTemplate = () => {
    const headers = ['empid', 'name', 'date', 'domain'];
    const sampleData = [
