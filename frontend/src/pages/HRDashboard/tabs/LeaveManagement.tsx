...
export default function LeaveManagement() {
  const { user } = useAuthStore(); // HR user logged in
  ...
  const mapDtoToUI = (dto: LeaveRequestDTO): LeaveRequest => {
    const employeeName =
      dto.user?.name || dto.user?.email || `User-${dto.user?.id || 'N/A'}`;
    return {
      id: String(dto.id),
      employee_id: String(dto.user?.id ?? ''),
  ...
  const loadLeaves = async () => {
    ...
    try {
      ...
      const data = res.data || [];
      const mapped = data.map(mapDtoToUI);
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-semibold text-gray-900 dark:text-white">
                          {leave.employee_name} <span className="text-sm text-gray-500 dark:text-gray-400">({leave.employee_id})</span>
                        </p>
                        <p className="text-sm text-gray-600 dark:text-gray-400">
                          {new Date(leave.from_date).toLocaleDateString()} - {new Date(leave.to_date).toLocaleDateString()} ({leave.days} days)
                        </p>
                      </div>
                    </div>
